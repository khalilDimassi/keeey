...
const ContactForm = ({ setFormActive, handleNewContact, isConsultant }: { setFormActive: (active: boolean) => void, handleNewContact: (role: contactRole, data: contact) => void, isConsultant: boolean }) => {
  const [role, setRole] = useState<contactRole>(isConsultant ? 'CONSULTANT' : 'INTERNAL-CONTACT');
  ...
  const getFormTitle = () => {
    if (role === 'CONSULTANT') return 'Consultant à suivre';
    return role === 'INTERNAL-CONTACT' ? 'Contact interne' : 'Contact externe';
  };

  const shouldShowCompanyField = () => {
    return role === 'EXTERNAL-CONTACT' || role === 'CONSULTANT';
  };
        ...
        >
          ...
          <div className="flex gap-4">
            <input
              ...
              name="occupation"
              className={`border rounded-xl px-3 py-2 text-sm ${shouldShowCompanyField() ? 'w-1/2' : 'w-full'}`}
              onChange={() => setFormTouched(true)}
            />
