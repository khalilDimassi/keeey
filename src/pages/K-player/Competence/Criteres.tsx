...
import { OpportunityFormData } from "./CompetencesEtCriteres";
...
const Criteres: FC<CriteresProps> = ({ formData, onFormDataChange }) => {
  ...
  const addItem = (field: keyof typeof inputValues, listKey: keyof OpportunityFormData) => {
    ...
    onFormDataChange({
      ...formData,
      [listKey]: [...currentArray, value]
    });

    handleInputChange(field, "");
  };

  const removeItem = (listKey: keyof OpportunityFormData, item: string) => {
    onFormDataChange({
      [listKey]: Array.isArray(formData[listKey]) ? formData[listKey].filter(i => i !== item) : formData[listKey],
    });
  };
    ...
    >
        ...
        <div className="flex gap-2">
          <input
            ...
            onChange={(e) => {
              ...
              handleFieldChange("crit_duration", value === "" ? null : parseInt(value, 10));
            }}
          />
        </div>
        <div className="flex gap-2">
          <input
            ...
            onChange={(e) => {
              const value = e.target.value;
              handleFieldChange("crit_duration_lastest", value === "" ? null : parseInt(value, 10));
            }}
          />
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-4">
        <input
          ...
          onChange={(e) => {
            const value = e.target.value;
            handleFieldChange("crit_target_rate", value === "" ? null : parseInt(value, 10));
          }}
        />
        <input
          type="text"
          ...
          onChange={(e) => {
            const value = e.target.value;
            handleFieldChange("crit_max_rate", value === "" ? null : parseInt(value, 10));
          }}
        />
      </div>
      ...
      <input
        ...
        onChange={(e) => handleFieldChange("crit_location", e.target.value)}
      />
        ...
        {[
          ...
          <label key={label} className="flex items-center gap-2">
            <input
              ...
              onChange={() => handleFieldChange("crit_remote", value)}
              className="w-4 h-4 text-blue-600"
            />
            {label}
          </label>
