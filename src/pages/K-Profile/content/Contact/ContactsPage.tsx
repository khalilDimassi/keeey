...
import { createContact, loadContacts } from "./services";
...
const ContactsPage = () => {
  ...
  useEffect(() => {
    const fetchContacts = async () => {
      setError(null);
      setLoading(true);
      const [data, error] = await loadContacts('REFERRAL');
      if (error) {
        setError(error);
      } else if (data) {
        setContacts(data);
          ...
          <div className="min-w-full">
            {activeTab === 'referrals' ? (
              <ContactsReferencesContent
                ...
                loading={loading}
                error={error}
                onReload={() => loadContacts('REFERRAL')}
