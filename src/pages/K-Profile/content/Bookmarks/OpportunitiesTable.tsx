...
const OpportunitiesTable: FC<OpportunitiesTableProps> = ({ onClose }) => {
  ...
  return (
    <div className="min-h-screen w-full flex flex-col">
          ...
          {loading ? (
            ...
            <>
              ...
              {opportunities.length < 12 &&
                [...Array(12 - opportunities.length)].map((_, index) => (
                  <tr key={`empty-${index}`} className="h-[46px]">
                    {[...Array(12)].map((_, cellIndex) => (
                      <td key={`empty-cell-${cellIndex}`} className="px-4 py-2 border-b border-gray-200">
                        &nbsp;
                      </td>
                    ))}
                  </tr>
      ...
      <JobOpportunities
        selectedID={selectedID ? selectedID : undefined}
        onClose={closeModal}
      />
    </div>
  );
