...
const Formation = ({ data }: { data: Training[] }) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setTrainings((prevTrainings) => {
          const existingIndex = prevTrainings.findIndex((t) => t.id === updatedTraining.id);
          if (existingIndex !== -1) {
            return prevTrainings.map((t) => (t.id === updatedTraining.id ? updatedTraining : t));
          }
          return [...prevTrainings, updatedTraining];
        });

        setNewTraining({ id: 0, name: "", description: "", organization: "", city: "", started_at: "", ended_at: "", present: false });
      }
    } catch (error) {
  ...
  const handleDelete = async (id: number) => {
    try {
      await axios.put(`${import.meta.env.VITE_API_BASE_URL}/api/v1/private/resume/training`, { id, name: null, description: null, organization: null, city: null, started_at: null, ended_at: null }, {
        headers: { "Content-Type": "application/json", "Authorization": getAuthHeader().Authorization },
      });
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        {trainings.map((training) => (
          <div key={training.id} className="flex justify-between items-center border border-gray-200 p-3 rounded-md">
            <span>{training.name}</span>
            <div className="flex gap-2">
              <button className="text-gray-500 hover:text-gray-700" onClick={() => handleUpdate(training)}><Pencil size={18} /></button>
              <button className="text-red-500 hover:text-red-700" onClick={() => handleDelete(training.id)}><Trash size={18} /></button>
            </div>
          </div>
        ))}
      </div>
